...
/*
   Versión hecha por Luis Miguel Guirado Bautista
   en el curso 2021-22
*/
...
double calcular_integral_secuencial(  )
{
   ...
   //cout << "Fin del calculo secuencial" << endl;
}
...
{
   //cout << "Comienza la hebra " << i << endl;
   double suma_parcial = 0;
   // Calculamos la suma parcial de las m/n barras que le corresponden
   for (long j=0; j < m/n; j++) {
      double xj;
      //xj = double(i*(m/n) + j + 0.5)/m; // Valor contiguo
      xj = double(i + j*n + 0.5)/m; // Valor entrelazado
      suma_parcial += f(xj);
   }

   //cout << "Hebra " << i << " ha acabado" << endl;
...
{
   //cout << "Calculo concurrente" << endl;
   double suma_total = 0;
   future<double> hilos[n];

   for (int i=0; i < n; i++)
      hilos[i] = async(launch::async, funcion_hebra, i);

   for (int i=0; i < n; i++)
      suma_total+= hilos[i].get();


   //cout << "Fin del calculo concurrente" << endl << endl;
