...
//
// Archivo: filosofos-nointerb.cpp
// Implementación del problema de los filósofos (sin camarero).
// Versión sin interbloqueo
//
...
void funcion_filosofos( int id )
{
  ...
  while ( true ) {
      ...
      // 1. SOLICITAR TENEDORES
      if ( aleatorio<0,100>() % 2 == 0 ) {
         cout <<"Filósofo " <<id << " solicita ten. izq." <<id_ten_izq <<endl;
         MPI_Ssend( &valor, 1, MPI_INT, id_ten_izq, etiq_ocup, MPI_COMM_WORLD);
         cout <<"Filósofo " <<id <<" solicita ten. der." <<id_ten_der <<endl;
         MPI_Ssend( &valor, 1, MPI_INT, id_ten_der, etiq_ocup, MPI_COMM_WORLD);
      }
      else {
         cout <<"Filósofo " <<id <<" solicita ten. der." <<id_ten_der <<endl;
         MPI_Ssend( &valor, 1, MPI_INT, id_ten_der, etiq_ocup, MPI_COMM_WORLD);
      ...
      if ( aleatorio<0,100>() % 2 == 0 ) {
         ...
         MPI_Ssend( &valor, 1, MPI_INT, id_ten_izq, etiq_lib, MPI_COMM_WORLD);
         cout<< "Filósofo " <<id <<" suelta ten. der. " <<id_ten_der <<endl;
         MPI_Ssend( &valor, 1, MPI_INT, id_ten_der, etiq_lib, MPI_COMM_WORLD);
      }
      else {
         cout<< "Filósofo " <<id <<" suelta ten. der. " <<id_ten_der <<endl;
         MPI_Ssend( &valor, 1, MPI_INT, id_ten_der, etiq_lib, MPI_COMM_WORLD);
